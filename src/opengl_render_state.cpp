#include "../include/openglUtils/sample_math.h"
#include "../include/openglUtils/opengl_render_state.h"
#include <iostream>
#include <stdexcept>
...
{
  OpenGlMatrix ret;
  MatMul<4,4,4>(ret.m, lhs.m, rhs.m);
  return ret;
}
...
{
#ifndef HAVE_GLES
  const FGLTools::OpenGlMatrix projmattrans = GetProjectiveTextureMatrix().Transpose();
...
OpenGlMatrix ModelViewlookAtRH_1(const Eigen::Vector3f & eye, const Eigen::Vector3f& center,  const Eigen::Vector3f& up)
    {
        Eigen::Vector3f f =(center).normalized();
        Eigen::Vector3f s =(up.cross(f)).normalized();
        Eigen::Vector3f u =-(f.cross(s));
