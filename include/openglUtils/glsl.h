...
class GlSlProgram {
 ...
 protected:
  ...
  static std::map<GlSlShaderType, std::string> SplitAnnotatedShaders(const std::string& code);
...
inline bool IsLinkSuccessPrintLog(GLhandleARB prog) {
  ...
  if (status != GL_TRUE) {
    printf("GLSL Program link failed: ");
...
inline void GlSlProgram::PreprocessGLSL(
  ...
  while (!input.eof()) {
    ...
    if (!strncmp(line, "#include", 8)) {
      ...
      if (ifs.good()) {
        const std::string file_path = PathParent(resolved_file);
  ...
  if (shader_file.code.empty()) {
    ...
    if (ifs.is_open()) {
      PreprocessGLSL(ifs, buffer, shader_file.program_defines, shader_file.search_path, ".");
          ...
          printf("Unable to open shader file '%s'", shader_file.filename.c_str());
    }
  } else {
...
inline bool GlSlProgram::AddShaderFromFile(
  ...
  ShaderFileOrCode shader_file = {
      shader_type, PathExpand(filename), std::string(), program_defines, search_path};
...
inline bool GlSlProgram::AddShader(
    ...
    const std::vector<std::string>& search_path) {
  ShaderFileOrCode shader_file = {
      shader_type, std::string(), source_code, program_defines, search_path};
  shader_files.push_back(shader_file);
  return AddShaderFile(shader_file);
}
...
inline std::map<GlSlShaderType, std::string> GlSlProgram::SplitAnnotatedShaders(
  ...
  while (!input.eof()) {
    ...
    if (!strncmp(line, "@start", 6)) {
      const std::string str_shader_type = Trim(std::string(line).substr(6));
...
inline void
GlSlProgram::SetUniform(const std::string& name, float f1, float f2, float f3, float f4) {
